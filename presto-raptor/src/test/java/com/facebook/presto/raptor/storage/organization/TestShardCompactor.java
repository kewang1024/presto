...
public class TestShardCompactor
{
            ...
            throws IOException
    {
        ...
        uuidsMap.forEach((uuid, deltaUuid) -> {
            try (ConnectorPageSource pageSource = getPageSource(storageManager, columnIds, columnTypes, uuid, deltaUuid, false)) {
                MaterializedResult result = materializeSourceDataStream(SESSION, pageSource, columnTypes);
                rows.rows(result.getMaterializedRows());
            }
            catch (IOException e) {
    ...
    private ConnectorPageSource getPageSource(StorageManager storageManager, List<Long> columnIds, List<Type> columnTypes, UUID uuid, Optional<UUID> deltaShardUuid, boolean deltaDeleteEnabled)
    {
