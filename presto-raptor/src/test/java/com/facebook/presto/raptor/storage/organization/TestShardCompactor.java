...
public class TestShardCompactor
{
    ...
    @Test(dataProvider = "useOptimizedOrcWriter")
    public void testShardCompactorWithDelta(boolean useOptimizedOrcWriter)
            throws Exception
    {
        StorageManager storageManager = createOrcStorageManager(dbi, temporary, MAX_SHARD_ROWS);
        List<Long> columnIds = ImmutableList.of(3L, 7L, 2L, 1L, 5L);
        ...
        long totalRows = inputShards.stream()
                .mapToLong(ShardInfo::getRowCount)
                .sum();
        long expectedOutputShards = computeExpectedOutputShards(totalRows - 2);
            ...
            throws IOException
    {
        ...
        for (UUID uuid : uuids) {
            try (ConnectorPageSource pageSource = getPageSource(storageManager, columnIds, columnTypes, uuid, Optional.empty(), Optional.empty())) {
            ...
            throws IOException
    {
        ...
        uuidsMap.forEach((uuid, deltaUuid) -> {
            try (ConnectorPageSource pageSource = getPageSource(storageManager, columnIds, columnTypes, uuid, deltaUuid, Optional.empty())) {
                MaterializedResult result = materializeSourceDataStream(SESSION, pageSource, columnTypes);
                rows.rows(result.getMaterializedRows());
            }
            catch (IOException e) {
    ...
    private ConnectorPageSource getPageSource(StorageManager storageManager, List<Long> columnIds, List<Type> columnTypes, UUID uuid, Optional<UUID> deltaShardUuid, Optional<Boolean> deltaDeleteEnabled)
    {
        return storageManager.getPageSource(uuid, deltaShardUuid, deltaDeleteEnabled, OptionalInt.empty(), columnIds, columnTypes, TupleDomain.all(), READER_ATTRIBUTES);
    }
