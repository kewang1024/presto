...
public class TestOrcStorageManager
{
            ...
            throws Exception
    {
        ...
        rowsToDelete.set(0);
        OriginalShardRewriter shardRewriter = (OriginalShardRewriter) manager.createShardRewriter(
                transactionId,
                OptionalInt.empty(),
            ...
            throws Exception
    {
        ...
        assertEquals(shardInfo.getRowCount(), 1);
        assertTrue(checkContent(shardInfo.getShardUuid(), rowsToDelete));
    ...
    private boolean checkContent(UUID shardUuid, BitSet rowsToDelete)
    {
        OrcStorageManager manager = createOrcStorageManager();
        Optional<BitSet> rows = manager.getRowsFromUuid(Optional.of(shardUuid));
        if (rows.isPresent()) {
    ...
    {
        return manager.getPageSource(uuid, Optional.empty(), false, OptionalInt.empty(), columnIds, columnTypes, tupleDomain, READER_ATTRIBUTES);
    }
