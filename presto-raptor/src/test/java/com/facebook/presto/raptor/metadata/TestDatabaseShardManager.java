...
public class TestDatabaseShardManager
{
            ...
            throws SQLException
    {
        long tableId = createTable("test");
        List<ColumnInfo> columns = ImmutableList.of(new ColumnInfo(1, BIGINT));
        shardManager.createTable(tableId, true, columns, false, OptionalLong.empty());
            ...
            throws SQLException
    {
        ...
        shardManager.replaceShardUuids(transactionId, true, tableId, columns, replaceUuidMap, ImmutableList.of(newShardInfo4), OptionalLong.of(0));
        ...
        Set<BucketShards> shardNodes = ImmutableSet.copyOf(shardManager.getShardNodes(tableId, true, TupleDomain.all()));
        ...
        // Verify statistics
        Statement stmt = dummyHandle.getConnection().createStatement();
        ResultSet resultSet = stmt.executeQuery("SELECT * FROM tables where table_id = " + tableId);
        resultSet.next();
    ...
    public void testReplaceShardUuids()
    {
        ...
        transactionId = shardManager.beginTransaction();
        shardManager.replaceShardUuids(transactionId, true, tableId, columns, replaceUuidMap, newShards, OptionalLong.of(0));
        ...
        try {
            ...
            Map<UUID, Pair<Optional<UUID>, Optional<ShardInfo>>> shardMap = ImmutableMap.of(originalUuids.get(0), new Pair(Optional.empty(), Optional.of(newDelta)));
        ...
        try {
            transactionId = shardManager.beginTransaction();
            Map<UUID, Pair<Optional<UUID>, Optional<ShardInfo>>> shardMap = ImmutableMap.of(originalUuids.get(0), new Pair(Optional.empty(), Optional.empty()));
            shardManager.replaceDeltaUuids(transactionId, tableId, columns, shardMap, OptionalLong.of(0));
            fail("expected exception");
        }
        catch (PrestoException e) {
            ...
            throws SQLException
    {
        ...
        // toReplace
        Map<UUID, Pair<Optional<UUID>, Optional<ShardInfo>>> shardMap = ImmutableMap.of(uuid4, new Pair<>(Optional.empty(), Optional.of(deltaInfo1)));
        transactionId = shardManager.beginTransaction();
        shardManager.replaceDeltaUuids(transactionId, tableId, columns, shardMap, OptionalLong.of(0));
    ...
    public void testReplaceDeltaUuids()
    {
        ...
        shardMap.put(originalUuids.get(0), new Pair(Optional.empty(), Optional.of(newDeltaShard1)));
        shardMap.put(originalUuids.get(1), new Pair(Optional.empty(), Optional.empty()));
        ...
        try {
            ...
            shardManager.replaceDeltaUuids(transactionId, tableId, columns,
                    ImmutableMap.of(originalUuids.get(0), new Pair(Optional.empty(), Optional.empty())), OptionalLong.of(0));
            fail("expected exception");
        }
        catch (PrestoException e) {
            assertEquals(e.getErrorCode(), TRANSACTION_CONFLICT.toErrorCode());
        ...
        shardMap = ImmutableMap.of(originalUuids.get(0), new Pair(Optional.of(newDeltaUuid1), Optional.of(shardInfo(anotherNewDeltaUuid1, nodes.get(0)))));
        shardManager.replaceDeltaUuids(transactionId, tableId, columns, shardMap, OptionalLong.of(0));
        ...
        shardNodes = ImmutableSet.copyOf(shardManager.getShardNodes(tableId, true, TupleDomain.all()));
        ...
        shardManager.replaceShardUuids(transactionId, true, tableId, columns, replaceUuidMap, ImmutableSet.of(shardInfo(uuid4, nodes.get(0))), OptionalLong.of(0));
    ...
    public void testEmptyTableBucketed()
    {
        long tableId = createTable("test");
        List<ColumnInfo> columns = ImmutableList.of(new ColumnInfo(1, BIGINT));
        shardManager.createTable(tableId, false, columns, true, OptionalLong.empty());
    ...
    public void testTemporalColumnTableCreation()
    {
        ...
        shardManager.createTable(tableId, false, columns, false, OptionalLong.of(1));

        long tableId2 = createTable("test2");
        List<ColumnInfo> columns2 = ImmutableList.of(new ColumnInfo(1, TIMESTAMP));
        shardManager.createTable(tableId2, false, columns2, true, OptionalLong.of(1));
    }
    ...
    public void testMaintenanceBlocked()
    {
        ...
        try {
            ...
            shardManager.replaceShardUuids(transactionId, false, tableId, columns, oldShardMap, ImmutableSet.of(), OptionalLong.empty());
            fail("expected exception");
        }
        catch (PrestoException e) {
            assertEquals(e.getErrorCode(), TRANSACTION_CONFLICT.toErrorCode());
