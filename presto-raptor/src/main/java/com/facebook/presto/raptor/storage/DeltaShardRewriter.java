        ...
        implements ShardRewriter
{
    ...
    Collection<Slice> writeDeltaDeleteFile(BitSet rowsToDelete)
    {
        ...
        if (oldDeltaDeleteShardUuid.isPresent()) {
            mergeRowsToDelete(rowsToDelete, oldDeltaDeleteShardUuid.get());
        }
        ...
        StoragePageSink pageSink = orcStorageManager.createStoragePageSink(transactionId, bucketNumber, columnIds, columnTypes, true);
    ...
    private void mergeRowsToDelete(BitSet rowsToDelete, UUID oldDeltaDeleteShardUuid)
    {
        Optional<BitSet> rowsDeleted = orcStorageManager.getRowsFromUuid(Optional.of(oldDeltaDeleteShardUuid));
    ...
    {
        OrcDataSource dataSource = orcStorageManager.openShard(oldShardUuid, defaultReaderAttributes);
        try {
