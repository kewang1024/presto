...
public final class ShardCompactor
{
    ...
    private List<ShardInfo> compact(StoragePageSink storagePageSink, boolean tableSupportsDeltaDelete, OptionalInt bucketNumber, Map<UUID, Optional<UUID>> uuidsMap, List<Long> columnIds, List<Type> columnTypes)
            throws IOException
    {
        for (Map.Entry<UUID, Optional<UUID>> entry : uuidsMap.entrySet()) {
            UUID uuid = entry.getKey();
            Optional<UUID> deltaUuid = entry.getValue();
            try (ConnectorPageSource pageSource = storageManager.getPageSource(FileSystemContext.DEFAULT_RAPTOR_CONTEXT, uuid, deltaUuid, tableSupportsDeltaDelete, bucketNumber, columnIds, columnTypes, TupleDomain.all(), readerAttributes)) {
    ...
    public List<ShardInfo> compactSorted(long transactionId, boolean tableSupportsDeltaDelete, OptionalInt bucketNumber, Map<UUID, Optional<UUID>> uuidsMap, List<ColumnInfo> columns, List<Long> sortColumnIds, List<SortOrder> sortOrders)
            throws IOException
    {
        ...
        try {
            uuidsMap.forEach((uuid, deltaUuid) -> {
                ConnectorPageSource pageSource = storageManager.getPageSource(FileSystemContext.DEFAULT_RAPTOR_CONTEXT, uuid, deltaUuid, tableSupportsDeltaDelete, bucketNumber, columnIds, columnTypes, TupleDomain.all(), readerAttributes);
