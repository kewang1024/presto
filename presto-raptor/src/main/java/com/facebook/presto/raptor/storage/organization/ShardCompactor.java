...
public final class ShardCompactor
{
            ...
            throws IOException
    {
        for (Map.Entry<UUID, Optional<UUID>> entry : uuidsMap.entrySet()) {
            ...
            try (ConnectorPageSource pageSource = storageManager.getPageSource(uuid, deltaUuid, tableSupportsDeltaDelete, bucketNumber, columnIds, columnTypes, TupleDomain.all(), readerAttributes)) {
            ...
            throws IOException
    {
        ...
        try {
            uuidsMap.forEach((uuid, deltaUuid) -> {
                ConnectorPageSource pageSource = storageManager.getPageSource(uuid, deltaUuid, tableSupportsDeltaDelete, bucketNumber, columnIds, columnTypes, TupleDomain.all(), readerAttributes);
