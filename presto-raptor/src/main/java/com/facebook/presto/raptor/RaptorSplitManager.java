            ...
            implements ConnectorSplitSource
    {
                ...
                boolean deltaDeleteEnabled)
        {
            ...
            if (bucketToNode.isPresent()) {
                iterator = shardManager.getShardNodesBucketed(tableId, deltaDeleteEnabled, merged, bucketToNode.get(), effectivePredicate);
            }
            else {
                iterator = shardManager.getShardNodes(tableId, deltaDeleteEnabled, effectivePredicate);
            }
        ...
        private ConnectorSplit createSplit(BucketShards bucketShards)
        {
            ...
            return new RaptorSplit(
                    connectorId,
                    shardUuid,
                    deltaShardUuid.isPresent() ? Optional.of(ImmutableMap.of(shardUuid, deltaShardUuid.get())) : Optional.empty(),
                    deltaDeleteEnabled,
                    addresses,
                    effectivePredicate,
                    transactionId,
