        ...
        implements StorageManager
{
            ...
            Optional<Map<String, Type>> allColumnTypes)
    {
        ...
        try {
            ...
            return new OrcPageSource(shardRewriter, recordReader, dataSource, columnIds, columnTypes, columnIndexes.build(), shardUuid, deltaDeleteEnabled, bucketNumber, systemMemoryUsage, rowsDeleted);
        }
    ...
    Optional<BitSet> getRowsFromUuid(Optional<UUID> deltaShardUuid)
    {
        if (!deltaShardUuid.isPresent()) {
            return Optional.empty();
        }
        try (OrcDataSource dataSource = openShard(deltaShardUuid.get(), defaultReaderAttributes)) {
            AggregatedMemoryContext systemMemoryUsage = newSimpleAggregatedMemoryContext();
    ...
    ShardRewriter createShardRewriter(long transactionId, OptionalInt bucketNumber, UUID shardUuid, Optional<UUID> deltaShardUuid, boolean deltaDeleteEnabled, Map<String, Type> columns)
    {
        if (deltaDeleteEnabled) {
            ...
            return new InplaceShardRewriter(transactionId, bucketNumber, shardUuid, columns, deletionExecutor, nodeId, this, storageService, shardRecorder, backupManager);
        }
    }
    ...
    ShardInfo createShardInfo(UUID shardUuid, OptionalInt bucketNumber, Path file, Set<String> nodes, long rowCount, long uncompressedSize)
    {
        try {
    ...
    OrcFileInfo rewriteFile(Map<String, Type> columns, Path input, Path output, BitSet rowsToDelete)
    {
        try {
