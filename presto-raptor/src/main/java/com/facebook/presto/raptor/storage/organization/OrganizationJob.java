        ...
        implements Runnable
{
    ...
    {
        try {
            runJob(organizationSet.getTableId(), organizationSet.isDeltaDeleteEnabled(), organizationSet.getBucketNumber(), organizationSet.getShardsMap());
        }
        catch (IOException e) {
    ...
    private void runJob(long tableId, boolean deltaDeleteEnabled, OptionalInt bucketNumber, Map<UUID, Optional<UUID>> shardUuidsMap)
            throws IOException
    {
        long transactionId = shardManager.beginTransaction();
        try {
            runJob(transactionId, deltaDeleteEnabled, bucketNumber, tableId, shardUuidsMap);
        }
        catch (Throwable e) {
    ...
    private void runJob(long transactionId, boolean deltaDeleteEnabled, OptionalInt bucketNumber, long tableId, Map<UUID, Optional<UUID>> shardUuidsMap)
            throws IOException
    {
        ...
        shardManager.replaceShardUuids(transactionId, deltaDeleteEnabled, tableId, metadata.getColumns(), shardUuidsMap, newShards, OptionalLong.empty());
    }
