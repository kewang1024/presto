        ...
        implements Runnable
{
    ...
    {
        try {
            runJob(organizationSet.getTableId(), organizationSet.getBucketNumber(), organizationSet.getShardsMap());
        }
        catch (IOException e) {
    ...
    private void runJob(long tableId, OptionalInt bucketNumber, Map<UUID, Optional<UUID>> shardUuidsMap)
            throws IOException
    {
        long transactionId = shardManager.beginTransaction();
        try {
            runJob(transactionId, bucketNumber, tableId, shardUuidsMap);
        }
        catch (Throwable e) {
    ...
    private void runJob(long transactionId, OptionalInt bucketNumber, long tableId, Map<UUID, Optional<UUID>> shardUuidsMap)
            throws IOException
    {
        ...
        shardManager.replaceShardUuids(transactionId, tableId, metadata.getColumns(), shardUuidsMap, newShards, OptionalLong.empty());
    }
